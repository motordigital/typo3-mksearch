 ...
 */
class tx_mksearch_scheduler_ElasticSearchCleanIndexTaskAddFieldProvider extends Tx_Rnbase_Scheduler_FieldProvider
{
    ...
    protected function _getAdditionalFields(array &$taskInfo, $task, $parentObject)
    {
        ...
        if (!array_key_exists(FIELD_ITEMS, $taskInfo) || empty($taskInfo[FIELD_ITEMS])) {
            if ($parentObject->CMD == 'add') {
                ...
                $taskInfo[FIELD_ITEMS] = $task->getUrl();
            } else {
        ...
        $fieldCode = '<input type="text" name="tx_scheduler['.FIELD_ITEMS.']" id="' . $fieldID .
                        '" value="' . $taskInfo[FIELD_ITEMS] . '" size="100" />';
        $additionalFields = array();
        $additionalFields[$fieldID] = array(
            'code'     => $fieldCode,
            'label'    => 'LLL:EXT:mksearch/locallang_db.xml:scheduler_elasticSearchCleanIndexTask_field_'.FIELD_ITEMS,
    ...
    {
        $task->setUrl($submittedData[FIELD_ITEMS]);
    }
}

if (defined('TYPO3_MODE') && $GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/mksearch/scheduler/class.tx_mksearch_scheduler_CleanIndexTaskAddFieldProvider.php']) {
    include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/mksearch/scheduler/class.tx_mksearch_scheduler_CleanIndexTaskAddFieldProvider.php']);
}
