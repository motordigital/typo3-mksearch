...
define('FIELD_ITEMS', 'cleanTables');
 ...
 */
class tx_mksearch_scheduler_CleanIndexTaskAddFieldProvider extends Tx_Rnbase_Scheduler_FieldProvider
{
    ...
    protected function _getAdditionalFields(array &$taskInfo, $task, $parentObject)
    {
        ...
        if (!array_key_exists(FIELD_ITEMS, $taskInfo) || empty($taskInfo[FIELD_ITEMS])) {
            if ($parentObject->CMD == 'add') {
                ...
                $taskInfo[FIELD_ITEMS] = $task->getCleanTables();
            } else {
        ...
        $additionalFields[$fieldID] = array(
            'code'     => $fieldCode,
            'label'    => 'LLL:EXT:mksearch/locallang_db.xml:scheduler_cleanIndexTask_field_'.FIELD_ITEMS,
    ...
    {
        $task->setCleanTables($submittedData[FIELD_ITEMS]);
    }
}
